...
class Jenis extends CI_Controller{
    ...
    function index($offset=0,$order_column='id_jenis',$order_type='asc'){
        if(empty($offset)) $offset=0;
        if(empty($order_column)) $order_column='id_jenis';
        if(empty($order_type)) $order_type='asc';
        
        //load data
        $data['jenis']=$this->m_jenis->semua($this->limit,$offset,$order_column,$order_type)->result();
        $data['title']="Data Jenis Koleksi";
    ...
    function tambah(){
        $data['title']="Tambah Jenis Koleksi";
        $this->_set_rules();
        if($this->form_validation->run()==true){//jika validasi dijalankan dan benar             
            if($this->m_jenis->validation($this->input->post('subject'))->num_rows == 0){
                $info=array(
                    'nama_jenis'=>$this->input->post('subject'),
                    
                );
                $this->m_jenis->simpan($info);
                redirect('jenis/index/add_success');
            } else {
                $data['message']="<div class='alert alert-danger'>Nama Jenis sudah ada</div>";
    ...
    function edit($id){
        $data['title']="Edit data jenis koleksi";
        $this->_set_rules();
    ...
    function hapus(){
        $kode=$this->input->post('kode');
        $detail=$this->m_jenis->cek($kode)->result();
    ...
    function _set_rules(){
        $this->form_validation->set_rules('subject','Nama Jenis Koleksi','required');
    }
}